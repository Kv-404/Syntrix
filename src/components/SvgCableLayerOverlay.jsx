...
export const SvgCableLayerOverlay = ({ activeCable }) => {
  ...
  useEffect(() => {
    ...
    const stageEl =
      document.querySelector(".patch-bay-stage") ||
  ...
  const renderCable = (startX, startY, endX, endY, color, id) => {
  ...
  const getCableColor = (sourceModuleId) => {
    ...
    <svg className="svg-cable-layer">
      {cables.map((cable) => {
        ...
        return renderCable(
          ...
          tgtPos.x,
          tgtPos.y,
          getCableColor(cable.sourceModuleId),
          cable.id,
        );
      })}
      ...
      {activeCable &&
        renderCable(
          portPositions[
            `port-${activeCable.sourceModuleId}-${activeCable.sourcePort}`
          ]?.x || activeCable.startX,
          portPositions[
            `port-${activeCable.sourceModuleId}-${activeCable.sourcePort}`
          ]?.y || activeCable.startY,
          activeCable.endX,
          activeCable.endY,
