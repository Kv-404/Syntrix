...
export class AudioEngine {
    ...
    useSynthStore.subscribe((state) => {
      ...
      if (outputModule) {
        this.masterGain.gain.setTargetAtTime(
          outputModule.parameters.masterGain || 0.8,
  ...
  noteOn(note) {
    ...
    const voiceContext = this.createVoiceGraph(
      state.modules,
      state.cables,
      frequency,
    );
    ...
    const now = this.audioContext.currentTime;
    voiceContext.modules.forEach((modNode) => {
      if (modNode.type === "ADSR") {
        const { attack, decay, sustain } = modNode.params;
        const gate = modNode.gateOutput;
  ...
  cleanupVoice(voiceContext) {
    if (voiceContext.cleanedUp) return;
    voiceContext.cleanedUp = true;
    voiceContext.nodes.forEach((node) => {
      try { node.stop(); } catch { /* node may not be stoppable */ }
  ...
  createVoiceGraph(modules, cables, frequency) {
    const nodes = [];
    ...
    modules.forEach((m) => {
      let audioNode;
      let auxNode; // e.g. a modulation input gain or constant source
    ...
    cables.forEach((cable) => {
      ...
      if (src && tgt) {
        ...
        if (tgt.type === "VCO" && cable.targetPort === "fmIn")
          targetNode = tgt.fmIn;
        if (tgt.type === "VCF" && cable.targetPort === "cvIn")
          targetNode = tgt.cvIn;
        if (tgt.type === "VCA" && cable.targetPort === "cvIn")
          targetNode = tgt.cvIn;
